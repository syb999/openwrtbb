...
static void parse_config(SSD1306_Config *config) {
    config->screen_on_time = DEFAULT_ON_TIME;
    config->screen_off_time = DEFAULT_OFF_TIME;
...
int main(int argc, char *argv[]) {
    SSD1306_Config config = {
        ...
        .screen_on_time = DEFAULT_ON_TIME,
        .screen_off_time = DEFAULT_OFF_TIME
    };
    ...
    while (running) {
        ...
        for (int i = 0; i < config.screen_on_time && running; i++) {
            ssd1306_clear(&dev);
